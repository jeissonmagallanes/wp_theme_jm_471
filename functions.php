 ...
 */
function cultura_logo_url() {
    return home_url();
}
add_filter( 'login_headerurl', 'cultura_logo_url' );

function cultura_logo_url_title() {
    return 'Powered by Cultura';
}
add_filter( 'login_headertitle', 'cultura_logo_url_title' );

function cultura_login_logo() { ?>
    <style type="text/css">
	    .login h1{
	    	margin-left: 0px;
	    }
...
<?php }
add_action( 'login_head', 'cultura_login_logo' );


/**
 * Remove admin bar
 */
function cultura_remove_admin_bar() {
	show_admin_bar(false);
	if ( current_user_can( 'administrator' ) ) {
		show_admin_bar(true);
	}
}
add_action( 'after_setup_theme', 'cultura_remove_admin_bar' );
 ...
 */
function cultura_generate_svg( $svg_mime ) {
	$svg_mime['svg'] = 'image/svg+xml';
	return $svg_mime;
}
add_filter( 'upload_mimes', 'cultura_generate_svg' );

/**
 * Enables the Excerpt meta box in Page edit screen.
 */
function cultura_add_excerpt_support_for_pages() {
	//add_post_type_support( 'page', 'excerpt' );
	//add_post_type_support( 'ai1ec_event', 'excerpt' );
}
add_action( 'init', 'cultura_add_excerpt_support_for_pages' );

/**
 * Function Add Icon Font
 */
function cultura_add_icon_fonts() {
...
add_action( 'wp_print_styles', 'cultura_add_icon_fonts' );

/**
 * Enqueue Schema org inline scripts
 */
function cultura_add_head_schema_script() {
	$options = cultura_get_trim_options( get_option( 'theme_settings' ) );
?>
...
add_action( 'wp_head', 'cultura_add_head_schema_script' );

/**
 * Function add url prop Schema
 */
function cultura_add_menu_attributes( $atts, $item, $args ) {
	$atts['itemprop'] = 'url';
	return $atts;
}
add_filter( 'nav_menu_link_attributes', 'cultura_add_menu_attributes', 10, 3 );

/**
 * Trim Theme options
 */
function cultura_get_trim_options( $arr_opts ){
	$opts = array();
 ...
 */
function cultura_body_tag_schema() {
 ...
 */
function cultura_has_shortcode_content( $shortcode = NULL ) {
 ...
 */
function cultura_get_pageid_by_template( $template ){
	$page_return = "";
 ...
 * Shortcode - Clear
 */
function cultura_shortcode_clear( $atts, $content = null ) {
...
add_shortcode( 'clear', 'cultura_shortcode_clear' );

/**
 * Shortcode - Button
 */
function cultura_shortcode_button( $atts, $content = null ) {
	$a = shortcode_atts( array(
		'class' => ''
	), $atts );
...
add_shortcode( 'button', 'cultura_shortcode_button' );
